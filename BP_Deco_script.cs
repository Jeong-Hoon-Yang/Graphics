...
public class BP_Deco_script : MonoBehaviour //장식품에 들어가는 코드
{
    RaycastHit hit;
    ...
    void Start()//가구 배치 모드일때 이 함수가 불린다
    {
        ...
        if (transform.position == Vector3.zero)//최초 포지션이 (0,0,0) 이면
        {
            ...
            //if (Physics.Raycast(ray, out hit, 50000.0f, (1 << 9)))//레이어 6에 레이를 쏨, 레이어6은 바닥,벽임
            //{
            //    transform.position = hit.point;
            //}
        }
    ...
    void Update()
    {
        ...
        //        if (tempTouch.phase == TouchPhase.Moved)
        //        {
        //            if (!EventSystem.current.IsPointerOverGameObject())//터치한 것이 가구라면
        //            {
        //                if (Physics.Raycast(ray, out hit, 50000.0f, LayerMask))//Ground
        //                {
        //                    transform.position = hit.point;
        //                }
    ...
    {
        if (!(other.CompareTag("Furniture") || other.CompareTag("Ground"))) //Furniture와 Ground를 제외한 태그에 닿으면 빨개짐
        {
            mat = mesh.material;
    ...
    {
        if (!(other.CompareTag("Furniture") || other.CompareTag("Ground")))//Ground만 닿고 있으면 원래대로 돌아옴
        {
    ...
    public void OnMouseDrag()
    {
        ...
        //    if (Physics.Raycast(ray, out hit, 50000.0f, (1 << 6)))//레이어 6에 레이를 쏨
        //    {
